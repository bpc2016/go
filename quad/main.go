...
var (
	...
	perm [N] int
	position = 0
	rate = 32 	// how quickly we process, goes down to 20
...
func partialImage(lim int, base int, cond int) image.Image {
	if rate==0 {rate=256}
	...
	for k:=position; k<lim; k++ { 
		...
		px = x[r_k]; py = y[r_k]
		//if px>=hw || py>=hh {continue}
		if px<hw || py>=hh {continue}
		rx = x0 +float64(px)/width*side
		ry = y0 + float64(py)/height*side 
		z = complex(rx, ry) // Image point (px, py) represents complex value z.
		//img.Set(px, height-py, mandelbrot(z,iterations,cn)) //orientation: '-'
		//img.Set(px, hh-py, mandelbrot(z,iterations)) //orientation: '-'
		img.Set(px-hw, py, mandelbrot(z,iterations)) //orientation: '-'
	}
	return img
}
